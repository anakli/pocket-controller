...
def get_class_stats(socket, storageClass):
  ...
  pkt = msg_packer.pack(*sampleMsg)
  socket.sendall(pkt)
  data = socket.recv(RESP_LEN_IOCTL_BYTES + LONG + LONG) # allBlocks (LONG) + freeBlocks (LONG)
  resp_packer = struct.Struct(RESP_STRUCT_FORMAT_IOCTL + "qq")
  [length, ticket, type_, err, opcode, allBlocks, freeBlocks] = resp_packer.unpack(data)
  if err != 0:
...
def main():
  s = connect()
  datanodeIp = "10.1.88.82"
  datanodePort = 50030
  #dn_remove(s, datanodeIp, datanodePort)
  #get_class_stats(s, 0)
  weight = 0.1
  #send_weightmask(s, "/hello/test", [(calculate_datanode_hash(datanodeIp, datanodePort), weight), (calculate_datanode_hash(datanodeIp, datanodePort-10), weight)])
  send_weightmask(s, "/hello", [(calculate_datanode_hash(datanodeIp, datanodePort), weight)])
